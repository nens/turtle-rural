...
def calc_waterbezwaar(toetshoogte, ow_opp, tp_slope, winterpeil, waterstand):
    ...
    waterstand = waterstand / 100
    ow_opp = ow_opp * 10000
    tp_slope = tp_slope / 100
    if toetshoogte <> 99999:
...
def check_items(input_dict, fields_list):
    ...
    for field in fields_list:
        if not(input_dict.has_key(field)):
...
def main():
    try:
        ...
        if not(gp.exists(rr_peilgebieden_tbl)):
            log.error("tabel "+rr_peilgebieden_tbl+" does not exist!")
            sys.exit(5) 
        if not(gp.exists(rr_oppervlak_tbl)):
            log.error("tabel "+rr_oppervlak_tbl+" does not exist!")
            sys.exit(5)
        if not(gp.exists(rr_toetspunten_tbl)):
            log.error("tabel "+rr_toetspunten_tbl+" does not exist!")
            sys.exit(5) 
        if not(gp.exists(rr_resultaten_tbl)):
            log.error("tabel "+rr_resultaten_tbl+" does not exist!")
            sys.exit(5) 
        ...
        for id,row in gegevens.items():
            if row['tldhelling']==None:
                ...
                tp_slope = row['tldhelling']

            if row['winterpeil']==None:
                winterpeil = 0
            else:
                winterpeil = row['winterpeil']

            if row['openwat_ha']==None:
                ow_opp = 0
            else:
                ow_opp = row['openwat_ha']

            # bereken waterbezwaar inundatie stedelijk
            toetshoogte_i_st = row['mtgmv_i_st']
            ...
            toetshoogte_o_st = row['mtgmv_o_st']
            ...
            toetshoogte_i_hl = row['mtgmv_i_hl']
            ...
            toetshoogte_o_hl = row['mtgmv_o_hl']
            ...
            toetshoogte_i_ak = row['mtgmv_i_ak']
            ...
            toetshoogte_o_ak = row['mtgmv_o_ak']
            ...
            toetshoogte_i_gr = row['mtgmv_i_gr']
            ...
            toetshoogte_o_gr = row['mtgmv_o_gr']
        ...
        wbFields = {config.get('waterbezwaar', 'output_field_id'): {'type': 'TEXT', 'length': '30'},
                    config.get('waterbezwaar', 'output_field_wb_i_m3'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_i_ha'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_o_m3'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_o_ha'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_i_st_ha'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_i_hl_ha'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_i_ak_ha'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_i_gr_ha'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_o_st_ha'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_o_hl_ha'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_o_ak_ha'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_o_gr_ha'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_i_st_m3'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_i_hl_m3'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_i_ak_m3'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_i_gr_m3'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_o_st_m3'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_o_hl_m3'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_o_ak_m3'):{'type': 'DOUBLE'},
                    config.get('waterbezwaar', 'output_field_wb_o_gr_m3'):{'type': 'DOUBLE'}}

        for field_name, field_settings in wbFields.items():
            try: 
                if field_settings.has_key('length'):
                    log.info(" - add field %s to %s" % (field_name, os.path.basename(output_waterbezwaar_tbl)))
                    gp.AddField(output_waterbezwaar_tbl, field_name, field_settings['type'], '#', '#', field_settings['length'])
                else:
                    log.info(" - add field %s to %s" % (field_name, os.path.basename(output_waterbezwaar_tbl)))
                    gp.AddField(output_waterbezwaar_tbl, field_name, field_settings['type'])
            except:
                log.warning("Warning: add field failed "+field_name+", does it already exist?")
